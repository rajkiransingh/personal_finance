...
def get_last_year_sum(db, model, converted_amount, date_field, last_year_start, last_year_today):
...
def get_monthly_totals(db: Session, Model, converted_amount: Union[str, Any], date_field: str):
    """
...
def get_spending_by_category(db: Session, ExpenseModel, converted_amount):
    ...
    rows = (
        ...
        .join(ExpenseCategory, ExpenseModel.category_id == ExpenseCategory.expense_category_id)
...
class DashboardDataCalculator(BaseFetcher):
    ...
    def get_converted_amount(self, model, amount_field, currency_field, is_currency_id=False):
    ...
    def get_investment_data(self) -> dict:
        ...
        # ----- AMOUNTS CONVERTED -----
        income_converted_amount = self.get_converted_amount(Income, 'amount', 'currency')
        expense_converted_amount = self.get_converted_amount(Expense, 'amount', 'currency')
        stock_investment_converted_amount = self.get_converted_amount(StockInvestment, 'total_invested_amount',
                                                                      'currency_id', is_currency_id=True)
        bullion_investment_converted_amount = self.get_converted_amount(BullionInvestment, 'total_invested_amount',
                                                                        'currency_id', is_currency_id=True)
        mutual_fund_converted_amount = self.get_converted_amount(MutualFundInvestment, 'total_invested_amount',
                                                                 'currency_id',
                                                                 is_currency_id=True)
        crypto_converted_amount = self.get_converted_amount(CryptoInvestment, 'total_invested_amount', 'currency_id',
                                                            is_currency_id=True)
        real_estate_converted_amount = self.get_converted_amount(RealEstateInvestment, 'total_invested_amount',
                                                                 'currency_id',
                                                                 is_currency_id=True)

        # ========== INCOME ==========
        income_ytd = get_ytd_sum(self.db, Income, income_converted_amount, 'earned_date', year_start, today)
        income_last_year_to_date = get_last_year_sum(self.db, Income, income_converted_amount, 'earned_date',
                                                     last_year_start,
                                                     last_year_today)
        ...
        # ========== EXPENSE ==========
        expense_ytd = get_ytd_sum(self.db, Expense, expense_converted_amount, 'spent_date', year_start, today)
        expense_last_year_to_date = get_last_year_sum(self.db, Expense, expense_converted_amount, 'spent_date',
                                                      last_year_start,
                                                      last_year_today)
        ...
        expense_percentage_of_income = (expense_ytd / income_ytd) * 100 if income_ytd > 0 else 0
        ...
        investment_ytd = sum([
            get_ytd_sum(self.db, StockInvestment, stock_investment_converted_amount, 'investment_date', year_start,
                        today),
            get_ytd_sum(self.db, BullionInvestment, bullion_investment_converted_amount, 'investment_date', year_start,
                        today),
            get_ytd_sum(self.db, MutualFundInvestment, mutual_fund_converted_amount, 'investment_date', year_start,
                        today),
            get_ytd_sum(self.db, CryptoInvestment, crypto_converted_amount, 'investment_date', year_start, today),
            get_ytd_sum(self.db, RealEstateInvestment, real_estate_converted_amount, 'investment_date', year_start,
                        today),
        ])

        # Sum all investment types for last year
        investment_last_year_to_date = sum([
            get_last_year_sum(self.db, StockInvestment, stock_investment_converted_amount, 'investment_date',
                              last_year_start,
                              last_year_today),
            get_last_year_sum(self.db, BullionInvestment, bullion_investment_converted_amount, 'investment_date',
                              last_year_start,
                              last_year_today),
            get_last_year_sum(self.db, MutualFundInvestment, mutual_fund_converted_amount, 'investment_date',
                              last_year_start,
                              last_year_today),
            get_last_year_sum(self.db, CryptoInvestment, crypto_converted_amount, 'investment_date', last_year_start,
                              last_year_today),
            get_last_year_sum(self.db, RealEstateInvestment, real_estate_converted_amount, 'investment_date',
                              last_year_start,
                              last_year_today),
        ])
        ...
        investment = {
            ...
            "investment_ratio": (investment_ytd / income_ytd * 100 if income_ytd else 0),
        ...
        try:
            ...
            total_income_till_today = get_ytd_sum(self.db, Income, income_converted_amount, 'earned_date',
                                                  date(1970, 1, 1), today)
        except Exception:
            total_income_till_today = 0

        try:
            total_expenses_till_today = get_ytd_sum(self.db, Expense, expense_converted_amount, 'spent_date',
                                                    date(1970, 1, 1),
                                                    today)
        except Exception:
        ...
        try:
            total_invested_till_today = (
                    get_ytd_sum(self.db, StockInvestment, stock_investment_converted_amount, 'investment_date',
                                date(1970, 1, 1), today)
                    + get_ytd_sum(self.db, BullionInvestment, bullion_investment_converted_amount, 'investment_date',
                                  date(1970, 1, 1), today)
                    + get_ytd_sum(self.db, MutualFundInvestment, mutual_fund_converted_amount, 'investment_date',
                                  date(1970, 1, 1), today)
                    + get_ytd_sum(self.db, CryptoInvestment, crypto_converted_amount, 'investment_date',
                                  date(1970, 1, 1), today)
                    + get_ytd_sum(self.db, RealEstateInvestment, real_estate_converted_amount, 'investment_date',
                                  date(1970, 1, 1),
                                  today)
            )
        except Exception:
        ...
        cash_balance = total_income_till_today - total_expenses_till_today - total_invested_till_today
        ...
        try:
            ...
            last_6_months_expense = get_ytd_sum(self.db, Expense, expense_converted_amount, 'spent_date',
                                                start_n_months_ago, today)
        except Exception:
        ...
        emergency_coverage = (cash_balance / emergency_required * 100) if emergency_required > 0 else 0
        ...
        # ========== SPENDING CATEGORIES ==========
        spending_categories = get_spending_by_category(self.db, Expense, expense_converted_amount)

        # ========== MONTHLY TRENDS ==========
        monthly_trends = {
            "income": get_monthly_totals(self.db, Income, income_converted_amount, 'earned_date'),
            "expense": get_monthly_totals(self.db, Expense, expense_converted_amount, 'spent_date'),
        }

        # investment per-type and aggregated
        investment_models = [
            ("Stocks", StockInvestment, stock_investment_converted_amount, 'investment_date'),
            ("Bullion", BullionInvestment, bullion_investment_converted_amount, 'investment_date'),
            ("MutualFund", MutualFundInvestment, mutual_fund_converted_amount, 'investment_date'),
            ("Crypto", CryptoInvestment, crypto_converted_amount, 'investment_date'),
            ("RealEstate", RealEstateInvestment, real_estate_converted_amount, 'investment_date'),
        ]
        ...
        aggregated_investment = [
            {"month": m, "total": round(agg_map[m], 2)}
            for m in sorted(agg_map.keys())
        ]
