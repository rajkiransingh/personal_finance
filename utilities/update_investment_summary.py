...
from backend.models.models import StockInvestment, MutualFundInvestment, StockSummary, MutualFundSummary, CryptoSummary, \
    BullionSummary, Dividends, DividendSummary
from backend.services.db_services import get_db
from utilities.metal_rate_fetcher import get_current_metal_rates
from utilities.mutual_fund_price_fetcher import get_mutual_fund_rates_bulk, get_mutual_fund_nav_dict
...
def update_stock_prices(db, common_stock_prices, dividend_stock_prices, dividends_data):
    ...
    try:
        ...
        for investment in stock_investments:
            ...
            stock_price_data = dividend_stock_prices.get(
                investment.stock_symbol) if investment.dividend_paying == 1 else common_stock_prices.get(
            ...
            if isinstance(stock_price_data, dict) and "currentPrice" in stock_price_data:
                ...
                if current_price == 'N/A':
                    logger.warning(
                        f"Unable to fetch the price for {investment.stock_symbol}, switching to previous closed price")
...
def update_investment_calculations(investment, current_price, dividend_amount, today):
    ...
    investment.return_on_investment = (((
                                                current_value + dividend_amount) - initial_investment) / initial_investment) * 100

    # Calculate XIRR (simplified version)
    days_invested = (today - investment.investment_date).days
    years = days_invested / 365.0

    investment.xirr = ((((current_value + dividend_amount) / initial_investment) ** (
    ...
    try:
        ...
        for investment in mf_investments:
            ...
            if scheme_code in mf_navs:
                ...
                if current_nav == "N/A" or isinstance(current_nav, str) and not current_nav.replace('.', '',
                                                                                                    1).isdigit():
                ...
                logger.info(
                    f"Got the total quantity: {investment.unit_quantity} and current price: {investment.current_price_per_unit}")
                investment.current_price_per_unit = current_nav
                investment.current_total_value = float(investment.current_price_per_unit) * float(
                    investment.unit_quantity)
                ...
                # Log the update
                logger.info(
                    f"Updated {scheme_code} - Current NAV: {current_nav}, Total Value: {investment.current_total_value}, ROI: {investment.return_on_investment:.2f}%, XIRR: {investment.xirr:.2f}%")
...
def get_data_from_investment_summary(db: Session):
    return {
        "common_stocks": [s.stock_symbol for s in db.query(StockSummary).filter(
            or_(StockSummary.dividend_paying == 0, StockSummary.dividend_paying == None)).all()],
