...
def update(db: Session, investment: RealEstateInvestmentCreate):
    property_to_update = db.query(RealEstateSummary).filter(
    ...
    if property_to_update:
        if investment.transaction_type == "BUY":
            ...
            property_to_update.total_quantity -= investment.area_in_sqyds
            property_to_update.total_cost -= (property_to_update.average_price_per_unit * investment.area_in_sqyds)
        ...
        property_to_update.average_price_per_unit = property_to_update.total_cost / property_to_update.total_quantity if property_to_update.total_quantity > 0 else 0
