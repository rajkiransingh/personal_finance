...
from backend.schemas.investment_schemas import CreateRealEstateInvestmentResponse, InvestmentUpdate
from backend.services.db_services import get_db
from backend.services.user_services import get_user
from backend.services.investment_services import get_all_investments, get_investment_by_user, get_investment_by_id, create_property, update_investment, delete_investment
from backend.summarizing import update_real_estate_summary

router = APIRouter(prefix="/investment/real-estate", tags=["Investment"])
@router.get("/", response_model=list[CreateRealEstateInvestmentResponse])
...
@router.get("/user/{user_id}", response_model=list[CreateRealEstateInvestmentResponse])
...
@router.get("/{investment_id}", response_model=CreateRealEstateInvestmentResponse)
...
@router.post("/", response_model=CreateRealEstateInvestmentResponse)
def add_investment_transaction(investment_data: CreateRealEstateInvestmentResponse, db: Session = Depends(get_db)):
    new_investment = create_property(investment_data, db)
